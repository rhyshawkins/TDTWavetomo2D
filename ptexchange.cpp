...
PTExchange::step()
{
  ...
  if (chain_rank == 0) {
    ...
    if (send) {
      ...
      sendmsg[3] = global.random.uniform();
      
      if (MPI_Send(sendmsg, 4, MPI_DOUBLE, partner, 0, temperature_communicator) != MPI_SUCCESS) {
      	throw WAVETOMO2DEXCEPTION("Failed to send current state to partner seconday\n");
      }      

      if (MPI_Recv(recvmsg, 4, MPI_DOUBLE, partner, 0, temperature_communicator, MPI_STATUS_IGNORE) != MPI_SUCCESS) {
    ...
    ptaccept = log(u) < ((((global.current_likelihood + global.current_log_normalization) -
			   (partner_likelihood + partner_log_normalization))/global.temperature) +
			 
     			 (((partner_likelihood + partner_log_normalization) -
			   (global.current_likelihood + global.current_log_normalization))/partner_temperature));
	...
	send_length = wavetree3d_sub_encode(global.wt, send_buffer, send_buffer_size);
  ...
  if (processesperchain > 1) {
	...
	if (wavetree3d_sub_decode(global.wt, send_buffer, send_length) < 0) {
	  throw WAVETOMO2DEXCEPTION("Failed to decode wavetree\n");
	}
      }
    }
