...
int main(int argc, char *argv[])
{
  ...
  GlobalSlice *global = new GlobalSlice(input_obs,
					initial_model_ptr,
					prior_file,
					degreex,
					degreey,
  ...
  ValueSlice *value = new ValueSlice(*global);

  HierarchicalSlice *hierarchical = nullptr;
  if (hierarchical_sigma > 0.0) {
    hierarchical = new HierarchicalSlice(*global, hierarchical_sigma);
  }

  HierarchicalPriorSlice *hierarchical_prior = nullptr;
  if (hierarchical_prior_sigma > 0.0) {
    hierarchical_prior = new HierarchicalPriorSlice(*global, hierarchical_prior_sigma);
  }

  PTExchangeSlice *ptexchange = nullptr;
  if (temperatures > 1) {
    ptexchange = new PTExchangeSlice(*global);
  }
  ...
  } catch (const wavetomo2dexception &e) {
    printf("Exception: see %s for details\n", logfile.c_str());
    return -1;
  };
...
static void usage(const char *pname)
{
  fprintf(stderr,
	  ...
	  " -H|--hierarchical-sigma <float> Hierarchical proposal std dev\n"
