...
struct user_data {
  ...
  double hierarchical_prior;

  FILE *fp_out;
  FILE *fp_hierarchical_out;
  FILE *fp_hierarchical_prior_out;
  FILE *fp_khistory_out;
...
static char short_options[] = "i:o:H:L:K:t:h";
static struct option long_options[] = {
  {"input", required_argument, 0, 'i'},
  {"output", required_argument, 0, 'o'},
  {"hierarchical", required_argument, 0, 'H'},
  {"hierarchical-prior", required_argument, 0, 'L'},
...
int main(int argc, char *argv[])
{
  ...
  while (1) {
    ...
    case 'L':
      hierarchical_prior_file = optarg;
      break;
  ...
  if (hierarchical_prior_file != NULL) {
    data.fp_hierarchical_prior_out = fopen(hierarchical_prior_file, "w");
    if (data.fp_hierarchical_prior_out == NULL) {
      fprintf(stderr, "error: failed to create hierarchical prior output file\n");
      return -1;
    }
  } else {
    data.fp_hierarchical_prior_out = NULL;
  }

  if (khistory_file != NULL) {
    data.fp_khistory_out = fopen(khistory_file, "w");
    if (data.fp_khistory_out == NULL) {
      fprintf(stderr, "error: failed to create khistory file\n");
      return -1;
    }
  } else {
    data.fp_khistory_out = NULL;
  }
  ...
  if (hierarchical_prior_file != NULL) {
    fclose(data.fp_hierarchical_prior_out);
  }


  if (khistory_file != NULL) {
    fclose(data.fp_khistory_out);
  }
		   ...
		   const multiset_int_double_t *S_v)
{
  ...
  if (step->header.type == CH_HYPER && step->header.accepted) {
    d->hierarchical_prior = step->perturbation.hyper.new_value;
  }
  ...
  if (d->thin <= 1 || (d->thincounter % d->thin == 0)) {
    ...
    if (d->fp_hierarchical_prior_out != NULL) {
      fprintf(d->fp_hierarchical_prior_out, "%.6g\n", d->hierarchical_prior);
    }

    if (d->fp_khistory_out != NULL) {
      fprintf(d->fp_khistory_out, "%d\n", multiset_int_double_total_count(S_v));
    }
...
static void usage(const char *pname)
{
  fprintf(stderr,
	  ...
	  " -L|--hierarchical-prior <file> Output hierarchical prior scale to file\n"
	  "\n"
	  " -t|--thin <int>                Thinning\n"
	  "\n"
