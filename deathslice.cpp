...
DeathSlice::DeathSlice(GlobalSlice &_global) :
  global(_global),
  propose(0),
  accept(0),
...
std::string
DeathSlice::write_short_stats()
{
  return mkformatstring("DeathSlice %6d/%6d %7.3f",
			accept,
			propose,
...
std::string
DeathSlice::write_long_stats()
{
  std::string s = mkformatstring("DeathSlice: %6d %7.3f:",
				 propose,
				 propose == 0 ? 0.0 : 100.0*(double)accept/(double)propose);
...
void
DeathSlice::initialize_mpi(MPI_Comm _communicator)
{
...
bool
DeathSlice::primary() const
{
  return (communicator == MPI_COMM_NULL || mpi_rank == 0);
}

int
DeathSlice::choose_death_location(int k,
			     double &ratio,
			     int &death_depth,
			     int &death_idx,
			     double &choose_prob,
...
int
DeathSlice::communicate_death_location(int &death_valid,
				  int &death_idx,
				  int &death_depth)
{
...
int
DeathSlice::propose_death(int death_valid,
		     int death_idx,
		     int death_depth,
		     double &death_value)
{
...
int
DeathSlice::compute_reverse_death_probability(int death_idx,
					 int death_depth,
					 double death_value,
					 int &ii,
					 int &ij,
...
int
DeathSlice::compute_likelihood(int death_idx,
			  double &proposed_likelihood,
			  double &proposed_log_normalization)
{
  if (communicator == MPI_COMM_NULL) {
...
int
DeathSlice::compute_acceptance(double proposed_likelihood,
			  double proposed_log_normalization,
			  double reverse_prob,
			  double choose_prob,
			  double death_prob,
...
int
DeathSlice::communicate_acceptance(bool &accept_proposal)
{
  if (communicator != MPI_COMM_NULL) {
