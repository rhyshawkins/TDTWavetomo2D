...
HierarchicalSlice::HierarchicalSlice(GlobalSlice &_global, double _sigma) :
  global(_global),
  sigma(_sigma),
  propose(0),
  accept(0),
...
HierarchicalSlice::~HierarchicalSlice()
{
}

int
HierarchicalSlice::step()
{
  propose ++;
...
std::string
HierarchicalSlice::write_short_stats()
{
  return mkformatstring("HierarchicalSlice %6d/%6d %7.3f",
			accept,
			propose,
...
std::string
HierarchicalSlice::write_long_stats()
{
  return write_short_stats();
}

void
HierarchicalSlice::initialize_mpi(MPI_Comm _communicator)
{
...
void
HierarchicalSlice::get_last_step(chain_history_change_t *_last_step)
{
  memcpy(_last_step, &last_step, sizeof(chain_history_change_t));
}

bool
HierarchicalSlice::primary() const
{
  return (communicator == MPI_COMM_NULL || mpi_rank == 0);
}

int
HierarchicalSlice::choose_value(double &value,
...
int
HierarchicalSlice::communicate_value(int &valid_proposal,
				double &value)
{
...
int
HierarchicalSlice::compute_likelihood(double value,
...
int
HierarchicalSlice::compute_acceptance(double log_value_prior_ratio,
				 double proposed_likelihood,
				 double proposed_log_normalization,
...
int
HierarchicalSlice::communicate_acceptance(bool &accept_proposal)
{
