...
BirthSlice::BirthSlice(GlobalSlice &_global) :
  global(_global),
  propose(0),
  accept(0),
...
std::string
BirthSlice::write_short_stats()
{
  return mkformatstring("BirthSlice %6d/%6d %7.3f",
			accept,
			propose,
...
std::string
BirthSlice::write_long_stats()
{
  std::string s = mkformatstring("BirthSlice: %6d %7.3f:",
				 propose,
				 propose == 0 ? 0.0 : 100.0*(double)accept/(double)propose);
  for (int i = 0; i <= global.treemaxdepth; i ++) {
...
void
BirthSlice::initialize_mpi(MPI_Comm _communicator)
{
...
bool
BirthSlice::primary() const
{
  return (communicator == MPI_COMM_NULL || mpi_rank == 0);
}

int
BirthSlice::choose_birth_location_and_value(int k,
					    double &ratio,
					    int &birth_depth,
					    int &birth_idx,
					    double &choose_prob,
...
int
BirthSlice::communicate_birth_location_and_value(int &birth_valid,
						 int &birth_idx,
						 int &birth_depth,
						 double &birth_value)
{
...
int
BirthSlice::propose_birth(int &birth_valid,
			  int &birth_idx,
			  int &birth_depth,
			  double &birth_value)
{
...
int
BirthSlice::compute_reverse_birth_probability(int birth_depth,
					      int birth_idx,
					      int ii,
					      int ij,
					      double birth_parent_coeff,
...
int
BirthSlice::compute_likelihood(int birth_idx,
			       double &proposed_likelihood,
			       double &proposed_log_normalization)
{
  if (communicator == MPI_COMM_NULL) {
...
int
BirthSlice::compute_acceptance(double proposed_likelihood,
			       double proposed_log_normalization,
			       double reverse_prob,
			       double choose_prob,
			       double birth_prob,
...
int
BirthSlice::communicate_acceptance(bool &accept_proposal)
{
