...
ValueSlice::ValueSlice(GlobalSlice &_global) :
  global(_global),
  propose(0),
  accept(0),
...
std::string
ValueSlice::write_short_stats()
{
  return mkformatstring("ValueSlice %6d/%6d %7.3f",
			accept,
			propose,
...
std::string
ValueSlice::write_long_stats()
{
  std::string s = mkformatstring("ValueSlice: %6d %7.3f:",
			       propose,
			       propose == 0 ? 0.0 : 100.0*(double)accept/(double)propose);
...
void
ValueSlice::initialize_mpi(MPI_Comm _communicator)
{
...
bool
ValueSlice::primary() const
{
  return (communicator == MPI_COMM_NULL || mpi_rank == 0);
}

int
ValueSlice::choose_value_location_and_value(int &value_depth,
					    int &value_idx,
					    double &choose_prob,
					    double &value,
					    int &ii,
...
int
ValueSlice::communicate_value_location_and_value(int &valid_proposal,
						 int &value_idx,
						 int &value_depth,
						 double &value)
{
...
int
ValueSlice::propose_value(int valid_proposal,
			  int value_idx,
			  int value_depth,
			  double value)
{
...
int
ValueSlice::compute_likelihood(int value_idx, double &proposed_likelihood, double &proposed_log_normalization)
{
  if (communicator == MPI_COMM_NULL) {
...
int
ValueSlice::compute_acceptance(int value_idx,
			  double value_prior_ratio,
			  double proposed_likelihood,
			  double proposed_log_normalization,
...
int
ValueSlice::communicate_acceptance(bool &accept_proposal)
{
